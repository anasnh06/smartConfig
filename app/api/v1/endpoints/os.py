...
def create_os(
    ...
    """
    os = os_service.create(os_in, created_by_id=current_user.id)
    return os_service.get_by_id(os.id, include_related=True)
...
def get_os(
    ...
    if not os:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Système d'exploitation introuvable.")
    return os
...
def update_os(
    ...
    if not updated:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Système d'exploitation introuvable.")
    return os_service.get_by_id(updated.id, include_related=True)
